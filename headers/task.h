...
namespace UHDM {
  ...
  public:
    ...
    unsigned int get_vpiType() { return vpiTask; }

    virtual unsigned int getUhdmType() { return uhdmtask; }   
  private:
  ...
  private:
    static std::vector<task*> objects_;
  };
 	      
  class VectorOftaskFactory {
  friend Serializer;
  public:
  static std::vector<task*>* make() {
    std::vector<task*>* obj = new std::vector<task*>();
    objects_.push_back(obj);
    return obj;
  }
  private:
  static std::vector<std::vector<task*>*> objects_;
  };
