...
std::string print_designs(const std::vector<vpiHandle>& designs) {
  ...
  for (auto restoredDesign : designs) {
    result +=
        "Design name: " + std::string(vpi_get_str(vpiName, restoredDesign)) +
        "\n";
    ...
    while (vpiHandle obj_h = vpi_scan(modItr)) {
      ...
      result +=
          ...
          ", top:" + std::to_string(vpi_get(vpiTopModule, obj_h)) +
          ", parent:" +
          std::string(vpi_get_str(vpiDefName, vpi_handle(vpiParent, obj_h))) +
      ...
      while (vpiHandle sub_h = vpi_scan(submodItr)) {
        result +=
            ...
            ", parent:" +
            std::string(vpi_get_str(vpiDefName, vpi_handle(vpiParent, sub_h))) +
      ...
      while (vpiHandle sub_h = vpi_scan(importItr)) {
        result += "\n    \\_ inst_item:" +
                  std::string(vpi_get_str(vpiDefName, sub_h));
        if (vpiFunction == vpi_get(vpiType, sub_h))
          result +=
              std::string(", size:") + std::to_string(vpi_get(vpiSize, sub_h));
      }
    ...
    while (vpiHandle obj_h = vpi_scan(pacItr)) {
      ...
      while (vpiHandle func_h = vpi_scan(funcItr)) {
        result += "  func: " + std::string(vpi_get_str(vpiName, func_h)) +
                  std::string(", size:") +
                  std::to_string(vpi_get(vpiSize, func_h));
        result += "\n";
      }
    }
  }
